...
<div class='containerx1'>
    ...
    <div class='containerx'>
        <ul class='responsive-table'><?php $ii = 0;
                                        while ($rows = $result->fetch_assoc()) {
                                            if ($ii == 0) { ?><div class='frstwrap'>
                        <div class='dtind'><?php echo date_format(date_create($rows['datetime']), 'd/m/y');
                                                $cur_date = date_format(date_create($rows['datetime']), 'd/m/y');
                                                $GLOBALS['frstdate'] = date_format(date_create($rows['datetime']), 'Y-m-d'); ?></div>
                        <div class='frto'>
                            <form action='?tr=done' method='POST'><input type='date' id='d1' name='d1'> to <input type='date' id='d2' name='d2'></form>
                        </div>
                    </div>
                    <div class='backk'><?php } else if ($cur_date !== date_format(date_create($rows['datetime']), 'd/m/y')) { ?></div>
                    <div class='dtind'><?php echo date_format(date_create($rows['datetime']), 'd/m/y');
                                                $cur_date = date_format(date_create($rows['datetime']), 'd/m/y');
                                                $GLOBALS['lastdate'] = date_format(date_create($rows['datetime']), 'Y-m-d'); ?></div>
                    <div class='backk'><?php }
                                            $ii = 1; ?><li class='table-row'>
                        <div class='col col-1'><?php echo date_format(date_create($rows['datetime']), 'h:i A'); ?></div>
                        <div class='col col-2'><?php echo $rows['s_name']; ?></div><?php $am = intval($rows['amount']);
                                                                                    if ($am > 0) {
